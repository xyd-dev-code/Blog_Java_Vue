...
// 未在后台填写关于页正文时，恢复原页面的默认展示内容。
// 这里只使用通用示例文案；真实姓名、邮箱和履历仍由 site_config 提供。
...
// 真实资料来自后端 site_config；未配置时仅使用不可投递的示例地址。
const siteEmail = computed(() => siteStore.info?.email?.trim() || 'site_email@example.com')
...
const roleTitle = computed(() => siteStore.info?.roleTitle?.trim() || '全栈工程师')
...
const timeline = computed(() => {
  ...
  if (!raw || !raw.trim()) return DEFAULT_TIMELINE
  const configured = raw.split(/\r?\n/).map(line => {
  ...
  return configured.length ? configured : DEFAULT_TIMELINE
})
...
onMounted(async () => {
  try {
    ...
    // 后台内容优先，未配置时使用原页面默认正文。
    let mdStr = siteStore.info?.aboutContent || ABOUT_MD
    if (greeting) {
