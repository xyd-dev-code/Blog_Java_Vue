...
- 目标：用户标记的文章右侧阅读区。沿用已有主题和系统字体，保留目录与正文交互。
- 修改 ArticleDetail.vue 局部样式：右侧内容最大宽度 880px；正文桌面 17px、移动端 16px、1.9 倍行距；两侧留白；更清晰的段落、列表、标题和引用间距；表格和代码保留滚动。
- 样式直接应用到 Markdown 预览层，防止第三方默认字号和行距覆盖阅读设置。
- 本轮没有改动目录逻辑、文章内容或全局主题样式。文件内此前已有的目录修复保留。
- 视觉验收阻塞：前轮浏览器工具报错所指的 browser-service.mjs 仍缺失，未获得实现截图，不宣称桌面、移动端和各主题已通过视觉验收。

---

# 访客明细：展开记录优化（2026-09-08，追加轮次）

final result: blocked

- 根据用户新提供的展开态截图，继续修改 Stats.vue：页面名称作为主信息、解码中文路径、超长路径使用原生 details 按需展开、时间精简到时分秒（分组顶部保留日期）、每位访客独立分页（8 条/页）及底部收起入口。
- 修正文章 slug 被标为“后端接口”、验证码缺少明确名称的问题。
- Node VM + Vue 响应式验证通过：中文解码、非法编码回退、保留 URI 分隔符编码、文章 slug/验证码名称、32 条记录分页、页码上下界、各访客独立页码、数据减少后页码收敛。
- 视觉验收沿用前轮阻塞：应用内浏览器运行组件缺失，未取得本轮实现截图，未宣称实际布局、键盘和响应式浏览器验收通过。
...
- 范围：`src/views/admin/Stats.vue` 的筛选、分组汇总、展开明细和分页；依据用户提供的 1920×869 截图优化，不做原图逐像素复刻。
- 已实现：带标签的响应式筛选、查询/重置、刷新归入标题区、访客与环境双行布局、完整地区换行、明确首次/最近时间、访问路径自适应高度、本页展开/收起、键盘操作及加载/空/失败状态。
- 数据限制：继续使用现有最多 500 条明细接口，在界面明确区分总记录数与已加载分组。
- 代码验证：`npm --prefix blog-web run build` 通过（现有依赖 PURE 注释警告）；`git diff --check -- blog-web/src/views/admin/Stats.vue` 通过。
- 行为验证：通过 Node VM 执行当前组件脚本、使用 Vue 实际响应式 API 和模拟请求，验证同 IP 跨日分组、首次/最近时间、组内降序、本页展开/收起、翻页清空、旧请求不覆盖新结果、刷新后页码收敛、重置筛选、空结果及错误状态，全部通过。此为脚本验证，未执行浏览器交互。
- 全局 `test:theme-layout` 未通过：断言比较前台组件与原始 sunny 布局，报告已有 CommentItem/CommentSection 等文件差异；该比较不包含本次修改的后台 Stats.vue，未更新基线或改动这些文件。
- 视觉验收阻塞：应用内浏览器启动报错，缺少 `./local-workspace`。本轮未取得实现截图，未验证桌面/移动端实际布局、主题对比度和实际浏览器交互，不宣称视觉验收通过。
